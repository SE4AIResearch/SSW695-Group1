...
func _ready() -> void:
	for i in SaveTool.get_save_list().size():
...
func _set_button_info(button: Button, save_name: String, summary: Dictionary) -> void:
	var saveInfo = button.get_node_or_null("saveInfo")
	if not saveInfo:
		return
	...
	if summary.exists:
		...
		saveInfo.text = "[center]\n\nEmpty Slot\n\n[/center]"

func _handle_save_selection(slot_index: int) -> void:
	var save_name = SaveTool.get_save_list()[slot_index]
...
func _handle_delete(slot_index: int) -> void:
	var save_name = SaveTool.get_save_list()[slot_index]
	SaveTool.delete_save(save_name)
	update_save_buttons()
